...
<div class="container mx-auto py-8 px-4">
    ...
    <!-- Filters -->
    <form method="GET" action="{{ route('activity_logs.index') }}" class="mb-8 space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="text" name="causer_name" id="causer_name"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                       value="{{ request('causer_name') }}" placeholder="Enter causer name">
            </div>
        ...
        <div class="flex items-center space-x-4">
            <button type="submit"
                    class="px-4 py-2 bg-blue-600 text-white font-semibold rounded-md shadow hover:bg-blue-700 focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                Filter
            </button>
            <a href="{{ url('/activity-logs') }}"
               class="px-4 py-2 bg-gray-500 text-white font-semibold rounded-md shadow hover:bg-gray-600 focus:ring-2 focus:ring-offset-2 focus:ring-gray-400">
                Reset
            </a>
        </div>
    </form>
    ...
    <div class="overflow-hidden shadow rounded-lg border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr>
                ...
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Properties
                </th>
            ...
            @forelse($logs as $log)
                <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">{{ $log->subject['name'] ?? class_basename($log->subject['type'] ?? '') }}
                        (ID: {{ $log->subject['id'] ?? '-' }})
                    </td>
                    <td class="px-4 py-2 whitespace-nowrap">
                        @if(!empty($log->properties))
                            <button onclick="openModal('propertiesModal{{ $log->_id }}')"
                                    class="text-blue-600 hover:underline">View
                            </button>

                            <!-- Modal -->
                            <div id="propertiesModal{{ $log->_id }}" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="flex items-center justify-center min-h-screen px-4">
                                    <div class="bg-white rounded-lg shadow-xl overflow-hidden max-w-fit">
                                        <div class="px-4 py-2 bg-gray-800 text-white text-lg font-semibold">Properties
                                        </div>
                                        <div class="p-4">
                                        ...
                                        <div class="px-4 py-2 bg-gray-100 text-right">
                                            <button onclick="closeModal('propertiesModal{{ $log->_id }}')"
                                                    class="px-4 py-2 bg-gray-600 text-white rounded-md">Close
                                            </button>
                                        </div>
                                    </div>
                                </div>
